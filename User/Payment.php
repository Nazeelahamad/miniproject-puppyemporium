...
session_start();
	...
	if(isset($_POST["btn_pay"])) {
		$a = "update tbl_request set request_status='3' where request_id='".$_GET["rid"]."'";
		if($con->query($a)) {
			// Get puppy ID from request
			$getPuppyIdQry = "SELECT puppy_id FROM tbl_request WHERE request_id = '".$_GET["rid"]."'";
			$puppyIdResult = $con->query($getPuppyIdQry);
			$puppyId = $puppyIdResult->fetch_assoc()['puppy_id'];
	
			// Decrease puppy stock by 1
			$updateStockQry = "UPDATE tbl_puppy SET puppy_stock = puppy_stock - 1 WHERE puppy_id = '$puppyId'";
			$con->query($updateStockQry);
			?>
			<script>
